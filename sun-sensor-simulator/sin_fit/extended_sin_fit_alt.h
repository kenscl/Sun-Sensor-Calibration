#ifndef __Alt_SIN_FIT
#define __Alt_SIN_FIT
#include <cstdint>
...
class Alt_sin_fit
{
    ...
    public:
         ...
         * the function that the data is fitted to is f(x) = a_1 * sin(a_2 * x) + b:1 * sign(x) * abs (x)^b_2 * sin (b_3 * abs(x)^b_4)
         ...
         */
        Alt_sin_fit(int64_t max_steps, double bound, std::vector<double> gt_data, std::vector<double> measurement);
        /*
